...
users = 512
iters = 1999
step_size = .00002  # adjust depending on users and iters (for 500 users, ~2000 iters: set to 20)
...
def utility(x, i, devices):
    ...
    for ind in range(len(devices)):
        if ind != i:
            price = price - price_weight * devices[ind].x
        else:
            price = price - price_weight * x
    return ut - price * x
