...
cpdef void run():
    # type definitions
    cdef int it, i, j
    mw = np.zeros((users,users))
    cdef double [:,:] metro_weights = mw
    # z_x = np.zeros(users-1)
    # cdef double [:] z_i_excl_self = z_x
    # oth_est = np.zeros(users - 1)
    # cdef double [:] others_estimates = oth_est
    ...
    for it in range(iters):
        ...
        adj_matrix = np.loadtxt('output/' + str(it + 1) + '/com_graph.csv', delimiter=',')
        a = 0
        for i in range(users):
            for j in range(users):
                a = i+j
                metro_weights[i][j] = metropolis_weight(i, j, adj_matrix, users=users)
        ...
        for i in range(users):
            ...
            new_action = new_action - step_size * gradient_objective(i, weighted_z, devices, quad_cost_coeff,
                                                                     lin_cost_coeff,
